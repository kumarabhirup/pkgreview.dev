export const meta = {
  title: 'pkgreview.dev',
  name: 'pkgreview.dev',
}

export const placeholderUserImage =
  'https://image.flaticon.com/icons/svg/25/25231.svg' // 'https://www.scrgrowthhub.co.uk/wp-content/uploads/placeholder-user-400x400-1.png'

export const featuredPackages = [
  {
    name: 'react',
    link: 'react',
    type: 'npm',
    image:
      'https://cdn4.iconfinder.com/data/icons/logos-3/600/React.js_logo-512.png',
  },
  {
    name: 'vue',
    link: 'vue',
    type: 'npm',
    image: 'https://vuejs.org/images/logo.png',
  },
  {
    name: 'jquery',
    link: 'jquery',
    type: 'npm',
    image:
      'https://www.shareicon.net/data/256x256/2015/08/30/93004_jquery_512x512.png',
  },
  {
    name: 'express',
    link: 'express',
    type: 'npm',
    image:
      'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTzfEbYVMe0L980MlsE4aBNyctaXwDnxbl370GtGrOmEVdFF7nW&s',
  },
  {
    name: 'lodash',
    link: 'lodash',
    type: 'npm',
    image: 'https://upload.wikimedia.org/wikipedia/en/6/6f/Lodash.svg',
  },
  {
    name: 'styled',
    link: 'styled-components',
    type: 'npm',
    image: 'https://miro.medium.com/max/652/1*N0XV3gco7Ed4brMoxwdjVg.png',
  },
  {
    name: 'koa',
    link: 'koa',
    type: 'npm',
    image:
      'https://raw.githubusercontent.com/github/explore/087f23463641d25ee971402fa26e3dfb2855edb9/topics/koa/koa.png',
  },
  {
    name: 'emotion',
    link: '@emotion%2Fcore',
    type: 'npm',
    image:
      'https://camo.githubusercontent.com/209bdea972b9b6ef90220c59ecbe66d35ffefa8a/68747470733a2f2f63646e2e7261776769742e636f6d2f746b6834342f656d6f74696f6e2f6d61737465722f656d6f74696f6e2e706e67',
  },
  {
    name: 'redux',
    link: 'redux',
    type: 'npm',
    image:
      'data:image/png;base64,',
  },
  {
    name: '⚛️ Spring',
    link: 'react-spring',
    type: 'npm',
    image:
      'https://spectrum.imgix.net/communities/ea41b01b-9774-4758-8373-0d3aac38b007/2db3a4dd-62fe-4243-b9d6-5be534165a1b-react-spring-logo.png?w=256&h=256&dpr=2&auto=compress&expires=1577664000000&ixlib=js-1.4.1&s=da5d556bf7ad9a8703a755bfe95c3e39',
  },
  {
    name: 'babel',
    link: '@babel%2Fcore',
    type: 'npm',
    image: 'https://img.icons8.com/dusk/2x/babel.png',
  },
  {
    name: 'gatsby',
    link: 'gatsby',
    type: 'npm',
    image:
      'https://res-3.cloudinary.com/crunchbase-production/image/upload/c_lpad,h_256,w_256,f_auto,q_auto:eco/pajbqv1x9twf1nh7lxpg',
  },
]
